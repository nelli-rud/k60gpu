...
void printGpuParameters(std::string prefixDescr) {	
	...
	for(int device = 0; device < deviceCount; device++){
		...
		ss << "Total Global Memory             : " << devProp.totalGlobalMem << " bytes ("<< devProp.totalGlobalMem
		...
		ss << "Max threads dim                 : " << devProp.maxThreadsDim[0] << " " << devProp.maxThreadsDim[1]
                                                     << " " << devProp.maxThreadsDim[2] << std::endl;
		ss << "Max threads per block           : " << devProp.maxThreadsPerBlock << std::endl;
		ss << "Max threads per multiprocessor  : " << devProp.maxThreadsPerMultiProcessor << std::endl;
		ss << "Max grid num                    : " << devProp.maxGridSize[0] << " " << devProp.maxGridSize[1]
